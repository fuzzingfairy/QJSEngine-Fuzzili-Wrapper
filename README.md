...
*Description*:
The Symbol.iterator provides a way to define the default iterator for an object. v5 stores the default iterator for v2 and when reduce is called, an attempt it made to reduce v7 to a single value according to the function provided by v5. Since v5 is an undefined function, the ArrayPrototype object tries to dereference it without making sure its not null
...
TODO: try filling the array to see if it stil crashes
*Description*:
...
*Similar Crashes Folder*:
`results/crashes/reviewed/other-invalid-iterator`
